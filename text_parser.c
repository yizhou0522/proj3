...
void parser(input parser_input) {
    ...
    if(parser_input.name != NULL){
        fptr = fopen(parser_input.name, "r");
        if(!fptr){
    ...
    // Check each line in the makefile. 
    // Print error message when violation of makefile format occurs.
    ...
    do {
        ...
        if (index >= MAX) {
            fprintf(stderr, 
            "%d: <Exceeds the max line number %d>: %s\n", curLine, MAX, line);
            free(line);
            exit(1);
        } else {
            line[index - 1] = '\0';
    ...
    if(parser_input.spec_targets[0] == NULL){
        ...
        for (unsigned int x = 0; x < MAX; x++) {
            ...
            if (!targetFound) {
                fprintf(stderr, "Fail to find target %s", 
                parser_input.spec_targets[x]);
            }
        }
    }
    if (fclose(fptr)) {
